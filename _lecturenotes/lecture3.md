...
Now suppose that $$w\notin S$$.  Then $$S$$ and $$V(\Gamma)\setminus S$$ are both nonempty <small>($$u \in S$$ and $$w \notin S \implies w \in V(\Gamma)) \setminus S$$)</small>, and by the above there are no edges between them, and so $$\Gamma$$ is not connected, a contradiction.
